...
async function loadUISettings() {
  ...
  try {
    var profileData = await new Promise((resolve) => {
      getUserProfile(userId, resolve)
      getUserProfile(userId,(profile)=>{
        console.log(
        profile )
        profile = JSON.parse(profile)
      profile.uiSettings = localStorage.getItem("USER_PROFILE").uiSettings
        console.log("233")
        console.log(profile.uiSettings)
    })
    }     
    );
    if (profileData) {
      var profile = JSON.parse(profileData);
      var settings = profile.uiSettings || {};
settings = localStorage.getItem("USER_PROFILE").uiSettings
      // Apply settings if they exist
      if (settings.cornerRadius) {
        document.documentElement.style.setProperty(
          "--user-corner-radius",
          `var(${settings.cornerRadius})`
        );
      }
      ...
      if (settings.backgroundColor) {
        ...
        if (settings.backgroundColor.startsWith("var(--bg-color-")) {
          var activePreset = document.querySelector(
            `[data-color="${settings.backgroundColor}"]`
          );
          if (activePreset) {
            ...
            var computedColor = getComputedStyle(document.documentElement)
              .getPropertyValue(
                settings.backgroundColor.replace("var(", "").replace(")", "")
              )
              .trim();
          }
        }
      ...
      if (settings.accentColor) {
        ...
        if (settings.accentColor.startsWith("var(")) {
          var activePreset = document.querySelector(
            `[data-color="${settings.accentColor}"]`
          );
          if (activePreset) {
            ...
            var computedColor = getComputedStyle(document.documentElement)
              .getPropertyValue(
                settings.accentColor.replace("var(", "").replace(")", "")
              )
              .trim();
  ...
  if (userId) {
    try {
      ...
      await getUserWordList(userId, (data) => {
        if (data) {
          var firebaseWords = JSON.parse(data);
          var localWords = JSON.parse(
            localStorage.getItem("favoriteWords") || "[]"
          );

          // Merge and deduplicate
          var mergedWords = [...firebaseWords, ...localWords].reduce(
            (acc, word) => {
      ...
      await getUserCollections(userId, (data) => {
        if (data) {
          var firebaseCollections = JSON.parse(data);
          var localCollections = JSON.parse(
...
async function containsWord(wordToCheck) {
  ...
  return localFavoriteWordsCache.some(
    (favWord) => favWord.word === wordToCheck
  );
}
...
function populateUI(wordObject) {
  // Reset speaker state
  endSpeaker();
  var speakerBtn = document.getElementById("speakerButton");
  speakerBtn.classList.add("material-symbols-rounded");
  speakerBtn.classList.remove("selected-material-symbol");

  // Prepare new content
  var newWord = `${wordObject.word[0].toUpperCase()}${wordObject.word
    .slice(1)
    .toLowerCase()}`;
  var newDef = `${wordObject.def[0].toUpperCase()}${wordObject.def
    .slice(1)
    .toLowerCase()}`;
  var newSyllables = splitIntoSyllables(wordObject.word);
  ...
  var favoriteBtn = document.getElementById("favorateWordButton");
...
function splitIntoSyllables(word) {
  ...
  var vowels = "aeiouy";
  var vowelRegex = new RegExp(`[${vowels}]`, "g");
...
function selectAndDisplayWord(direction = "f") {
  ...
  playSound("click");
  var nextWord = getNextWord(direction);
...
async function toggleFavoriteWord() {
  var currentWord = wordList[place];
  ...
  try {
    var isCurrentlyFavorited = localFavoriteWordsCache.some(
    ...
    localStorage.setItem(
      "favoriteWords",
      JSON.stringify(localFavoriteWordsCache)
    );
...
var speakWord = () => {
  ...
  var wordUtterance = new SpeechSynthesisUtterance(
    document.getElementById("word").textContent
  );
  var defUtterance = new SpeechSynthesisUtterance(
...
function copyAndShowAlert() {
  var currentWord = wordList[place];
  var shareword = `${currentWord.word} - ${currentWord.partOfSpeech}\n${currentWord.def}\n\nShared from Verbi`;
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      var shareBtn = document.getElementById("shareWordButton");
  ...
  try {
    ...
    if (collection) {
      if (!collection.words.some((w) => w.word === wordData.word)) {
        ...
        var modalsToRemove = document.querySelectorAll(".modal.active");
...
async function showCreateCollectionModal() {
  ...
  var saveBtn = modal.querySelector("#saveCollection");
...
function shareWord() {
  var currentWord = wordList[place];
  var shareword = `Check out this word on Verbi: ${currentWord.word} - ${currentWord.def}`;
